...
def _llm_answer(job: Job, message: str, cases: list[dict], entries: list[KnowledgeEntry], manuals: list[dict], history: list[AssistantTurn]) -> str:
...
def run_unified_turn(db: Session, payload) -> dict:
    ...
    answer = _llm_answer(job, payload.message, cases, entries, manuals, history)
    ...
    db.commit()
    return {"conversation_id": conversation.id, "job_id": job.id, "answer": answer, "intent": intent, "created_work_order": created, "similar_cases": cases, "knowledge_entries": entries, "actions": ["created_work_order" if created else "updated_work_order", "searched_verified_cases", "searched_knowledge", "searched_manuals", "saved_conversation"]}
